            ...
            <div className="flex flex-row justify-between">
              ...
              <div className="flex flex-row space-x-4">
                <Link href={listState === 'draft' ? '/?type=normal' : '/?type=draft'}
                      className={`text-sm mt-4 text-neutral-500 font-normal ${listState === 'draft' ? "text-primary" : ""}`}>
                  {t('draft_bin')}
                </Link>
                <Link href={listState === 'unlisted' ? '/?type=normal' : '/?type=unlisted'}
                      className={`text-sm mt-4 text-neutral-500 font-normal ${listState === 'unlisted' ? "text-primary" : ""}`}>
                  {t('unlisted')}
                </Link>
              </div>
              }
          ...
          <Waiting for={status === 'idle'}>
            ...
            <div className="w-[80vw] flex flex-row items-center mt-4 ani-show">
              ...
              <Link href={`/?type=${listState}&page=${(page - 1)}`}
                    className={`text-sm font-normal rounded-full px-4 py-2`}>
                {t('previous')}
              </Link>
              }

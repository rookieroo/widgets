...
import {Bolt, SquareMousePointer} from "lucide-react";
...
async function publish({
                         ...
                         url,
                         showAlert
                       }: { name: string, avatar: string, desc: string, url: string, showAlert: ShowAlertType }) {
  const t = i18next.t
...
export function FriendsPage() {
  ...
  useEffect(() => {
    ...
    client.friend.index.get({
      ...
      if (data) {
        const friends_available = data.friend_list?.filter(({
                                                              health,
                                                              accepted
                                                            }) => health.length === 0 && accepted === 1) || []
        shuffleArray(friends_available)
        setFriendsAvailable(friends_available)
        const friends_unavailable = data.friend_list?.filter(({
                                                                health,
                                                                accepted
                                                              }) => health.length > 0 && accepted === 1) || []
  ...
  return (<>
    ...
    <Waiting for={friendsAvailable.length !== 0 || friendsUnavailable.length !== 0 || status === "idle"}>
      <main className="w-full flex flex-col justify-center items-center mb-8 text-primary">
        ...
        <FriendList title={t('friends.my_apply')} show={profile?.permission !== true && apply !== undefined}
                    friends={apply ? [apply] : []}/>
...
function Friend({friend}: { friend: FriendItem }) {
  ...
  return (
    <>
      <a title={friend.name} href={friend.url} target="_blank"
         className="w-full rounded-xl p-4 flex flex-col justify-center items-center relative ani-show">
        <div className="w-16 h-16">
          <img className={"rounded-full " + (friend.health.length > 0 ? "grayscale" : "")} src={friend.avatar}
               alt={friend.name}/>
        </div>
        ...
        {friend.accepted !== 1 &&
        <p className={`${friend.accepted === 0 ? "text-primary" : "text-primary"}`}>{statusOption[friend.accepted + 1].label}</p>}
        ...
        {(profile?.permission || profile?.id === friend.uid) && <>
            <Button
                ...
                className="absolute top-0 right-0 m-2 px-2 py-1 text-primary rounded-full"
            >
                <Bolt className="h-4 w-4 dark:text-white"/>
            </Button></>}
      </a>
      <AlertDialog
        open={modalIsOpen}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t('update$sth', {sth: friend.name})}
            </AlertDialogTitle>
            <AlertDialogDescription>
              <div
                className="p-4 flex flex-col justify-start items-center relative">
                <div className="w-16 h-16">
                  <img className={"rounded-xl " + (friend.health.length > 0 ? "grayscale" : "")} src={friend.avatar}
                       alt={friend.name}/>
                </div>
                {profile?.permission &&
                <div
                    className="flex flex-col w-full items-start mt-4 px-4"
                >
                    <div className="flex flex-row justify-between w-full items-center">
                        ...
                        <div className="flex flex-row items-center justify-center space-x-4 text-primary">
                            ...
                            >
                                ...
                                <SelectContent>
                                    <SelectGroup>
                                      {statusOption.length > 0 && statusOption.map(op =>

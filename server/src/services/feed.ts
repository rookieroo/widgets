import {and, count, desc, eq, like, ilike, or} from "drizzle-orm";
  ...
  return new Elysia({aot: false})
    ...
    .group('/feed', (group) =>
      group
        ...
        .get('/search', async ({admin, set, query: {title}}) => {
          const where = and(eq(feeds.draft, 0), eq(feeds.listed, 1), ilike(feeds.title, title));
          return (await db.query.feeds.findMany({
            where: where,
            columns: {
              id: true,
